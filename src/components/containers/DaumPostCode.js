...
function DaumPostCode(props) {
  const { setValue, errors, control } = props;
  const { daum } = window;
  ...
  function execDaumPostcode() {
    new daum.Postcode({
      oncomplete(data) {
        const { roadAddress, bname, buildingName, zonecode, jibunAddress, autoRoadAddress, autoJibunAddress, apartment } = data;
    ...
    <div>
      <Grid container spacing={1}>
        <Grid item xs={12}>
          <Grid container spacing={1}>
            <Grid item xs={2} onClick={execDaumPostcode}>
              <ReactFormText control={control} errors={errors} name="postcode" placeholder="우편번호" inputProps={{ readOnly: true }} />
            </Grid>
            <Grid item xs={2}>
              <StyledButton onClick={execDaumPostcode} height="40px">
                우편번호 찾기
              </StyledButton>
            </Grid>
          </Grid>
        ...
        <Grid item xs={12}>
          <Grid container spacing={1}>
            <Grid item xs={4}>
              <ReactFormText control={control} errors={errors} name="detailAddress" placeholder="상세주소" />
            </Grid>
            <Grid item xs={4}>
              <ReactFormText control={control} errors={errors} name="extraAddress" placeholder="참고항목" inputProps={{ readOnly: true }} />
            </Grid>
          </Grid>
        </Grid>
      </Grid>
