...
function CopyDialog(props) {
  ...
  const { open, closeDialog, campaignId, getCampaigns } = props;
  ...
  return (
    <StyledDialog classes={{ paper: classes.paper }} fullScreen={fullScreen} open={open} onClose={close} aria-labelledby="responsive-dialog-title">
      <Box
        p="15px"
        fontSize="16px"
        fontWeight="400"
      ...
      <Box px={2} py={2} textAlign="center" boxSizing="border-box">
        ...
        <Box m="0 auto" mt={2} width={240}>
          <Grid container justifyContent="space-between">
            <Grid item>
              <Box p={1} border={`2px solid ${type === "1" ? "red" : "transparent"}`} onClick={() => setType("1")}>
                <StyledImage width="40px" height="40px" src={Instagram} />
              </Box>
            </Grid>
            <Grid item>
              <Box p={1} border={`2px solid ${type === "2" ? "red" : "transparent"}`} onClick={() => setType("2")}>
                <StyledImage width="40px" height="40px" src={Youtube} />
              </Box>
            </Grid>
            <Grid item>
              <Box p={1} border={`2px solid ${type === "3" ? "red" : "transparent"}`} onClick={() => setType("3")}>
        ...
        <Box mt={4}>
          <Box width="110px" m="0 auto">
            <Button fullWidth color="primary" variant="contained" disabled={type === null} onClick={copyCampaign}>
              복사
            </Button>
          </Box>
        </Box>
