            ...
            <Box ml="14px" height="100%">
              <Grid container alignContent="space-between" style={{ height: "100%" }}>
                <Grid item xs={12}>
                  ...
                  <StyledText fontSize="14px" color="#222">
                    {item.AD_TYPE === "1" ? <span style={{ color: Colors.pink, fontWeight: "600" }}>Instagram</span> : null}
                    {item.AD_TYPE === "2" ? <span style={{ color: Colors.red, fontWeight: "600" }}>Youtube</span> : null}
                    {item.AD_TYPE === "3" ? <span style={{ color: Colors.green, fontWeight: "600" }}>Blog</span> : null}
...
function HistoryDialog(props) {
  const { open, closeDialog, INF_ID } = props;
  ...
  function getHistory() {
    axios
      .get("/TB_PARTICIPANT/getHistory", { params: { INF_ID } })
      .then((res) => {
        const { data } = res.data;
        setCampaigns(data);
      })
  ...
  return (
    ...
    >
      ...
      <Box px={2} py={2} boxSizing="border-box">
        {loading ? <Box textAlign="center">Loading...</Box> : <List campaigns={campaigns} />}
        <Box mt={4}>
          <Box width="110px" m="0 auto">
            <Button fullWidth color="primary" variant="contained" onClick={close}>
              닫기
            </Button>
          </Box>
        </Box>
