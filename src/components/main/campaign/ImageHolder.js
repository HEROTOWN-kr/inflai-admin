...
function ImageHolder(props) {
  const { setValue, images, setImages, dbImages, setDbImages, campaignId } = props;
  ...
  async function getCampaignPhoto() {
    try {
      const response = await axios.get("/TB_PHOTO_AD/", { params: { id: campaignId } });
  ...
  function ImageActionButton(componentProps) {
    const { children, color, background, onClick, borderRadius, padding } = componentProps;
  ...
  function deleteDbPicture(id) {
    axios
      .post("/TB_PHOTO_AD/deleteImageAWS", { id })
      .then((res) => {
        getCampaignPhoto();
      })
      .catch((err) => {
  ...
  function setMainPicture(id) {
    axios
      .post("/TB_PHOTO_AD/setMain", { id, adId: campaignId })
      .then((res) => {
        getCampaignPhoto();
      })
      .catch((err) => {
                ...
                {dbImages.map((item) => (
                  <Grid item key={item.PHO_ID}>
                    <div style={{ position: "relative" }}>
                      <StyledImage width="130px" height="130px" borderRadius="12px" src={item.PHO_FILE_URL} />
                      <span
                        onClick={() => {
                          setImageId(item.PHO_ID);
                          setDialogOpen(true);
                        }}
                        style={deleteBtn}
                      >
                      ...
                      <span
                        onClick={() => setMainPicture(item.PHO_ID)}
                        style={{ ...mainImgButton, border: `4px solid ${item.PHO_IS_MAIN === 1 ? "#e03f3f" : "#dfe2e8"}` }}
                      >
                        button
                      </span>
                    </div>
          ...
          <Grid container spacing={2}>
            {images.map((item) => (
              <Grid item key={item.id}>
                <div style={{ position: "relative" }}>
                  <StyledImage width="130px" height="130px" borderRadius="12px" src={item.url} />
                  <span onClick={() => deletePicture(item.id)} style={deleteBtn}>
                    button
                  </span>
                  <span
      ...
      <Grid item xs={12}>
        <Grid container spacing={1}>
          ...
          {images.length > 0 ? (
            <React.Fragment>
              <Grid item>
                <ImageActionButton onClick={deleteSelected} padding="13px 22px">
                  선택삭제
                </ImageActionButton>
              </Grid>
              <Grid item>
                <ImageActionButton onClick={selectAll} padding="13px 22px">
                  모두선택
                </ImageActionButton>
              </Grid>
            </React.Fragment>
